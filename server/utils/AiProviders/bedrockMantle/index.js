 ...
 *
 * This is a separate provider from the legacy `bedrock` provider which uses the
 * AWS SDK (SigV4 / IAM roles). Mantle exposes an OpenAI-compatible API and an
 * Anthropic-compatible API, both authenticated with a single bearer API key.
 *
 * Both providers can be configured at the same time - they use entirely
 * separate environment variables.
 */
class AWSBedrockMantleLLM {
  ...
  constructor(embedder = null, modelPreference = null) {
    if (!process.env.AWS_BEDROCK_MANTLE_LLM_API_KEY)
      throw new Error(
        "AWS_BEDROCK_MANTLE_LLM_API_KEY is required for AWS Bedrock (Mantle)."
      );
    if (!process.env.AWS_BEDROCK_MANTLE_LLM_REGION)
      throw new Error(
        "AWS_BEDROCK_MANTLE_LLM_REGION is required for AWS Bedrock (Mantle)."
      );

    this.className = "AWSBedrockMantleLLM";
    this.model =
      modelPreference || process.env.AWS_BEDROCK_MANTLE_LLM_MODEL_PREFERENCE;
    this.region = process.env.AWS_BEDROCK_MANTLE_LLM_REGION;
  ...
  get #maxTokens() {
    return Number(process.env.AWS_BEDROCK_MANTLE_LLM_MAX_TOKENS) || 4096;
  }
  ...
  #log(text, ...args) {
    console.log(`\x1b[32m[AWSBedrockMantle]\x1b[0m ${text}`, ...args);
  }
  ...
  streamingEnabled() {
    if (!!process.env.AWS_BEDROCK_MANTLE_STREAMING_DISABLED) return false;
    return "streamGetChatCompletion" in this;
  }

  static promptWindowLimit() {
    const limit =
      process.env.AWS_BEDROCK_MANTLE_LLM_MODEL_TOKEN_LIMIT ?? 8191;
    const numericLimit = Number(limit);
    if (isNaN(numericLimit) || numericLimit <= 0) {
      this.#slog(
        `Invalid AWS_BEDROCK_MANTLE_LLM_MODEL_TOKEN_LIMIT: "${limit}". Returning default 8191.`
      );
      return 8191;
    }
    return numericLimit;
  ...
  promptWindowLimit() {
    return AWSBedrockMantleLLM.promptWindowLimit();
  }
  ...
  async getChatCompletion(messages = null, { temperature }) {
    if (!messages?.length)
      throw new Error(
        "AWSBedrockMantle::getChatCompletion requires a non-empty messages array."
      );
    ...
    const result = await LLMPerformanceMonitor.measureAsyncFunction(
      this.openai.chat.completions
        ...
        .catch((e) => {
          this.#log(
            `Bedrock Mantle API Error (getChatCompletion): ${e.message}`,
            e
          );
          throw new Error(
            `AWSBedrockMantle::getChatCompletion failed. ${e.message}`
          );
        })
    );
    ...
    if (!response?.choices?.[0]?.message) {
      this.#log("Bedrock Mantle response missing expected structure.", response);
      return null;
    }
  ...
  async streamGetChatCompletion(messages = null, { temperature }) {
    if (!Array.isArray(messages) || messages.length === 0)
      throw new Error(
        "AWSBedrockMantle::streamGetChatCompletion requires a non-empty messages array."
      );
    ...
    if (this.#isAnthropic) {
      ...
      // Prompt tokens are reported by the Anthropic stream itself, so we skip
      // the local token calculation here.
      return await LLMPerformanceMonitor.measureStream(stream, messages, false);
    }
  ...
  /**
   * `duration` from LLMPerformanceMonitor is already in seconds on this branch,
   * so outputTps is tokens / duration directly.
   */
  #buildMetrics(usage = {}, duration = 0, completionTokensOverride = null) {
    ...
    return {
      ...
      outputTps: completionTokens && duration ? completionTokens / duration : 0,
      duration,
    };
  }
